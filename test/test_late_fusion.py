...
from models.LateFusion import LateFusion
...
def parse_args():
    parser = argparse.ArgumentParser(f"{__file__}: --model \"late_fusion_mitbih|late_fusion_ptbdb\" --train|--valid|--test")
    parser.add_argument("--train", action='store_true', help='Set the flat to True')
    ...
    parser.add_argument("--model", nargs='*', type=str, default=["late_fusion_ptbdb"])
...
def main(cnn_config, transformer_config, local_config, hyper_tuning, load_weights, freeze):
    ...
    valid_models = ["late_fusion_mitbih", "late_fusion_ptbdb"]
    ...
    for m in model:
        ...
        # create a directory to save the late fusion model
        model_path = Path(local_config.saved_models)
        model_path.mkdir(parents=True, exist_ok=True)
        model_path = str(model_path.joinpath("late_fusion_"+data_name+".pth"))

        print(f"-----Test Late Fusion layer for: {data_name}------")
        model = LateFusion(model_1d_cnn_transformer, model_2d_cnn, num_classes=num_classes)
        criterion = None
        if is_reduce == True:
        ...
        if is_train == True:
            ...
            WR, WS = model.get_attention_weights()
            print("Attention Weights for Raw Signal (WR):", WR)
            print("Attention Weights for Spectrogram (WS):", WS)       
